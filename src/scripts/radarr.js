...
module.exports = {
  apiURL: function apiUrl (uri, params) {
    var str = Url.resolve(process.env.HUBOT_RADARR_HTTP, Path.join('api', uri));
    if (params) {
      str = str + '?' + QS.stringify(params);
    }
    return str;
  },
  fetchFromRadarr: function fetchFromRadarr (url) {
    return new Promise(function (resolve, reject) {
      var options = {
        url: url,
        headers: {
          'Accept': 'application/json',
          'X-Api-Key': process.env.HUBOT_RADARR_API_KEY
        }
      };
